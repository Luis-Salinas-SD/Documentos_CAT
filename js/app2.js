	...
	$(document).ready(function(){
		  ...
		  .done(function(listas_uni){
			$('#uni').html(listas_uni)
		  })
		  .fail(function(){
			alert('Hubo un error al cargar las Unidades')
		  })

		  $('#uni').on('change', function(){
				if (.val(unidad)=='203431000') {
				$.ajax({
			  type: 'POST',
			  url: 'usuario.php',
		})
			.done(function(listas_uni){
			  $('#usuario').html(listas_uni)
			})
			.fail(function(){
			  alert('Hubo un error al cargar los Analistas')
			})
		  })
		  })
			else{							  
			...
			'                                  ANALISTA:             ' + $('#usuario option:selected').text() )
